...
pub mod tests {
    ...
    use crate::MarkovChain as OldMarkovChain;

    #[test]
    pub fn matches_old_homogenous_chain() {
        let bond_energy = -3.0;
        let inert_fugacity = 1.0;
        let bonding_fugacity = 1.1;
        let inert_bonding_rate = 0.1;
        let chemical_potential = 2.3;

        let (width, height) = (20, 20);
        let odds = [
            (SiteState::Empty, 1.),
            (SiteState::Inert, 1.),
            (SiteState::Bonding, 2.0),
        ];
        let old_system = crate::System::random(width, height, odds);
        let old_chain = crate::HomogenousChain::beta_1(
            bond_energy,
            chemical_potential,
            inert_fugacity,
            bonding_fugacity,
        ...
        let allowed_reactions = old_chain.allowed_reactions(&old_system);

        for reaction_id in 0..chain.num_possible_reactions(&state) {
            let reaction = chain.reaction(&state, reaction_id);
            let old_reaction = if let BasicReaction::PointChange { from, to, position } = reaction {
                crate::Reaction::PointChange { from, to, position }
            } else {
                panic!("Non point change reaction: {:?}", reaction);
            };
            if chain.rate(&state, &boundary, reaction_id) == 0.0 {
                assert!(!allowed_reactions.contains(&old_reaction));
            } else {
                assert!(
                    chain.rate(&state, &boundary, reaction_id)
                        == old_chain.rate(&old_system, old_reaction)
                );
            }
        }
    }
